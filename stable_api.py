from broker10api.api import Broker10API
...
from broker10api.expiration import get_expiration_time, get_remaning_time
...
# --------for binary option detail
    ...
    def get_payout(self, par, timeframe):
        payout = 0
        if timeframe <= 1:
            try:
                p = self.get_binary_option_detail()
                payout = 100 - p[par]['turbo']['option']['profit']['commission']
            except:
                payout = 0
        else:
            try:
                p = self.get_binary_option_detail()
                payout = 100 - p[par]['binary']['option']['profit']['commission']
            except:
                payout = 0
        return payout

    def get_open_assets(self):
        abertos = self.get_all_open_time(0.1)
        abertosb = abertos["binary"]
        abertosd = abertos["digital"]
        parb = []
        pard = []
        for par in abertosb:
            if abertosb[par]["open"]:
                parb.append(par)
        for par in abertosd:
            if abertosd[par]["open"]:
                pard.append(par)
        abertos = {'digital': pard, 'binarias': parb}
        return abertos
    ...
    def get_binary_option_detail(self):
        ...
        for actives in init_info["result"]["turbo"]["actives"]:
            nome = init_info["result"]["turbo"]["actives"][actives]["ticker"]
...
# ______________________________________self.api.getprofile() https________________________________
    ...
    """def get_profile(self):
        while True:
            try:
                #logging.error('**warning** get_profile')
    ...
    def change_balance(self, Balance_MODE):
        self.account_type = Balance_MODE
        def set_id(b_id):
...
##############################################################################################
    ...
    def check_win(self, external_id, timeframe, polling_time=0.5):
        if not self.check_connect():
            self.connect()
            self.change_balance(self.account_type)

        max_time = int(time.time()) + ((timeframe*60)*2)
        ...
        def get_position_id():
            ...
            if check:
                ...
                while position_id==None and int(time.time()) < max_time:
                ...
                ok_id=external_id
            return ok_id
        while True and int(time.time()) < max_time:
            external_id=get_position_id()
            if external_id == None:
                if not self.check_connect():
                    self.connect()
                    self.change_balance(self.account_type)
                    saldo_now = self.get_balance()
                    result = saldo_now - self.saldo_start
                return result
...
# __________________FOR OPTION____________________________
    ...
    def buy_binary(self, ACTIVES, price, ACTION, expirations,polling=0.1):
        if self.check_connect():
            self.saldo_start = self.get_balance() 
        else:
            check, _ = self.connect()
            ...
            if check:
                ...
                return False, None        # Generate a unique request ID
        req_id = global_value.get_req_id(self.api.object_id)
        
        # Initialize buy_multi_option for this request
        if not hasattr(self.api, 'buy_multi_option'):
            self.api.buy_multi_option = {}
            
        self.api.buy_multi_option[req_id] = None
        
        # Symbol normalization logic (same as Polarium)
        if isinstance(ACTIVES, str):
            if "-OTC" not in ACTIVES:
                # For non-OTC symbols, try adding -op suffix first
                par = ACTIVES + "-op"
                if par not in OP_code.ACTIVES:
                    # If -op version not found, use original
                    par = ACTIVES
            else:
                # For OTC symbols, use exactly as provided
                par = ACTIVES
                
            # Check if the final symbol exists
            if par not in OP_code.ACTIVES:
                logging.error(f"ExNova symbol not found: {par} (original: {ACTIVES})")
                return False, f"Symbol not found: {par}"
                
            active_id = OP_code.ACTIVES[par]
            ACTIVES = par  # Update ACTIVES for logging
        else:
            active_id = ACTIVES
            
        logging.info(f"ExNova binary trade - Active: {ACTIVES}, ID: {active_id}")
        
        # Place the trade
        self.api.buyv3(price, active_id, ACTION, expirations, req_id)
        
        # Wait for the response with timeout
        start_time = time.time()
        timeout = 3  # 5 seconds timeout
        
        while self.api.buy_multi_option[req_id] == None:
            if time.time() - start_time > timeout:
                logging.error("ExNova trade execution timed out")
                return False, None
            time.sleep(0.02)
            
        # Check the response
        response = self.api.buy_multi_option[req_id]
        if isinstance(response, dict):
            if "message" in response:
                logging.error(f"ExNova trade failed: {response['message']}")
...
# __________________for Digital___________________
    ...
    def buy_digital(self, active, amount, action, duration):
        ...
        # Initialize digital_option_placed for this request if not exists
        if not hasattr(self.api, 'digital_option_placed'):
            self.api.digital_option_placed = {}
        ...
        # Format instrument ID for ExNova
        # Get the active ID, handle both string and integer inputs
        if isinstance(active, str):
            active_id = str(OP_code.ACTIVES[active])
        else:
            active_id = str(active)
            
        # Format IQ/Broker10 digital-spot: do{id}A{YYYYMMDD}D{HHMMSS}T{dur}M{C|P}SPT
        exp_dt = datetime.fromtimestamp(exp)
        instrument_id = ("do" + str(active_id) + "A" + exp_dt.strftime("%Y%m%d") +
                         "D" + exp_dt.strftime("%H%M%S") + "T" + str(duration) +
                         "M" + action + "SPT")
        
        logging.info(f"ExNova digital trade - Instrument ID: {instrument_id}")
         
        self.api.digital_option_placed[req_id] = None
        
        # Place the trade with timeout
        ...
        while self.api.digital_option_placed[req_id] == None:
            if time.time() - start_time > timeout:
                logging.error("Digital trade execution timed out")
                return False, None
            time.sleep(0.1)
        ...
        try:
            ...
            if isinstance(_tmp, dict) and "msg" in _tmp:
                logging.error(f"Digital trade failed: {_tmp}")
            else:
                logging.error(f"Unexpected response format: {_tmp}")
            return False, _tmp
            
        # Wait for position change with timeout
        start_time = time.time()
        while len(self.api.order_async[int(id)]["position-changed"]) == 0:
            if time.time() - start_time > timeout:
                logging.error("Digital trade position change timed out")
                return False, None
            time.sleep(0.1)
        ...
        try:
            ...
            logging.error("Failed to get digital trade external ID")
            return False, None
